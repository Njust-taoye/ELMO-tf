...
class ElmoKoreanDataset:
    ...
    def text_to_char_sequence(self, text):
        ...
        char_idx_seq, seq_len = self.char_vocab.to_seq(jamo_text, char_seq_len=self.char_seq_len, seq_len=self.seq_len,
                                                       with_len=True)
        seq_len = self.seq_len if seq_len > self.seq_len else seq_len
        return char_idx_seq, seq_len

    def text_to_word_sequence(self, text):
        word_idx_seq, seq_len = self.word_vocab.to_seq(text, seq_len=self.seq_len, with_len=True)
        seq_len = self.seq_len if seq_len > self.seq_len else seq_len
    ...
    def produce_data(self, text):
        ...
        return {"input": char_word_input, "input_len": input_len, "target": word_target, "target_len": target_len}
